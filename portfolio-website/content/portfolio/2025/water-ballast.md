+++
title = "Rocket Water Ballast System"
date = "2025-07-30"
description = "Engineered a post-apogee water ballast ejection system for a high-altitude competition rocket payload."
summary = "Designed and built a functioning water ballast payload for BYU Rocketry's Experimental High Power Team. The system ejected 2 kg of water during post-apogee descent and flew on BYU's first hybrid rocket launch. This project highlights systems integration, mechanical design, fabrication, and team-based aerospace development."
time_spent_hours = 50
role = "Payload Engineer"
team = "BYU Rocketry Experimental High Power Team"
duration = "September 2024 - April 2025"
outcome = "Delivered a flight-ready water ballast payload that successfully flew on BYU's first hybrid rocket launch and recovery."
core_tools = ["Onshape CAD", "OpenRocket", "3D Printing", "Carbon Fiber Fabrication", "Electronics Integration"]
header_alt = "BYU Rocketry's Category V hybrid rocket on the launch pad with the water ballast payload integrated inside."
social_image_alt = "Rocket Water Ballast System project showing a hybrid rocket payload developed for BYU Rocketry."

featured = true
weight = 6

[[project_links]]
label = "Team Page"
url = "https://rocketry.byu.edu/junior-high-power-team"
variant = "primary"
icon = "solid fa-rocket"
new_tab = true

[[project_links]]
label = "Press Coverage"
url = "https://www.ksl.com/article/51303782/byu-rocketry-club-reaches-new-heights-with-historic-launch-recovery-of-hybrid-rocket"
variant = "primary"
icon = "solid fa-newspaper"
new_tab = true
+++

## Overview

I followed a complete engineering design process to create a functioning water-ejection system for a high-altitude rocket. The system ejects **2 kg of water in 90 seconds** from a **6-inch-diameter** rocket during post-apogee descent.

I was one of four payload engineers on the [Experimental High Power Team](https://rocketry.byu.edu/junior-high-power-team). In addition to payload work, I also supported overall rocket creation, coordination, and launch preparation.

This was the first time BYU Rocketry launched a hybrid motor vehicle, and the launch was covered in [local news](https://www.ksl.com/article/51303782/byu-rocketry-club-reaches-new-heights-with-historic-launch-recovery-of-hybrid-rocket).

## Key Contributions

- Helped design and integrate a water ballast payload for BYU's first hybrid rocket
- Supported successful launch and recovery during the 2025 [FAR-OUT](https://www.faroutlaunch.org/) competition
- Assisted with fabrication and integration using a custom launch trailer and student-built rocket systems

## Timeline

- **Duration:** September 2024 - April 2025
- **Total time:** 50 hours
- **Time commitment:** 1-2 hours per week across two semesters

## Results

### Preparation and Design

In two technical design presentations (PDR and CDR), our rocket and payload designs were iteratively improved before construction began.

![](assets/water-ballast/Water1.jpg)
{{< caption >}}The Experimental High Power Team of 2024-2025{{< /caption >}}

![](assets/water-ballast/Water2.png)
{{< caption >}}Overview of rocket internals{{< /caption >}}

![](assets/water-ballast/Water3.png)
{{< caption >}}Design and analysis of the rocket in [OpenRocket](https://openrocket.info/){{< /caption >}}

Our initial design used a container of water pressurized by a motor-plunger system to eject the stored water.

![](assets/water-ballast/Water4.png)
{{< caption >}}CAD model of the water-ejection system{{< /caption >}}

### Videos

{{< callout title="Accomplishment" variant="success" >}}
This was the first Utah student-made hybrid rocket launch and recovery, and the project was featured in a [KSL article](https://www.ksl.com/article/51303782/byu-rocketry-club-reaches-new-heights-with-historic-launch-recovery-of-hybrid-rocket).
{{< /callout >}}

{{< youtube lLpFiryUqTQ >}}
{{< caption >}}Close-up view of launch{{< /caption >}}

{{< youtube VkSoiQ7n1To >}}
{{< caption >}}Rocket reaching an apogee of roughly 6,000 feet{{< /caption >}}

{{< youtube gIIojh048zM >}}
{{< caption >}}Hybrid Rocketry: BYU's Experimental Team Takes Flight{{< /caption >}}

### Final Product

The final design consisted of three primary components once removed from the rocket.

![](assets/water-ballast/Water6.jpg)
{{< caption >}}Complete water ballast system{{< /caption >}}

Component overview from left to right:

- Water bag for flexible storage of up to 3 kg of water
- Top bulkhead fitted with a solenoid for water-valve control
- Plunger and bottom bulkhead attached to the motor, battery, and electronics

The water bag is sandwiched between the two bulkheads inside the rocket. During post-apogee descent, an altimeter activates the motor, the plunger extends, and the water exits through the solenoid and tubing.

![](assets/water-ballast/Water5.jpg)
{{< caption >}}Top view of the water ballast system{{< /caption >}}

I also assisted with the production of carbon-fiber rocket tubes. The images below show the fully assembled and decorated rocket.

![](assets/water-ballast/header.PNG)
![](assets/water-ballast/Water8.PNG)
{{< caption >}}Rocket name: **CATEGORY V**{{< /caption >}}

## Technical Skills

{{< callout title="Engineering Design" variant="tip" >}}
Learned how to develop a new system through iterative design reviews, testing, and refinement.
{{< /callout >}}

{{< callout title="Manufacturing" variant="tip" >}}
Gained hands-on fabrication experience with 3D printing, carbon fiber, crimping, soldering, and machine tools such as lathes and mills.
{{< /callout >}}
